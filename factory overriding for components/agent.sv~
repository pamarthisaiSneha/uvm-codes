...
`uvm_component_utils(agent)
    ...
    function void build_phase(uvm_phase phase);
        ...
        `uvm_info(get_full_name(),"MESSAGE IS FROM BUILD PHASE WITH NONE",UVM_NONE)  
    
    endfunction

    function void connect_phase(uvm_phase phase);
        drv_h.seq_item_port.connect(sqr_h.seq_item_export);   // IN BUILT SEQUENCER PORT IT IS NOT REQUIRE TO DECLARE THEM 
        `uvm_info(get_full_name(),"MESSAGE IS FROM AGENT CONNECT PHASE WITH MEDIUM",UVM_MEDIUM)
    endfunction
